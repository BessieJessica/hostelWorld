    ...
    <div class="content">
        ...
        <div class="right-content">
            ...
            <div class="normal-div">我的会员卡号：
                <span class="number">${membership.memberCode}</span>

                <c:choose>
                    <c:when test="${membership.state == 0}">
                        <span class="number">[未激活]</span>
                        <button class="button" onclick="window.location.href='/validate'">立即激活</button>
                    </c:when>
                    <c:when test="${membership.state == 1}">
                        <span class="number">[有效]</span>
                        <button class="button" onclick="stop()">停止账户</button>
                    </c:when>
                    <c:when test="${membership.state == 2}">
                        <span class="number">[已暂停]</span>
                        <button class="button" onclick="window.location.href='/user/recharge'">恢复使用</button>
                        <button class="button" onclick="stop()">停止账户</button>
                    </c:when>
                    <c:when test="${membership.state ==3}">
                        <span class="number">[已停止]</span>
                    </c:when>
                </c:choose>
            </div>

            <c:if test="${membership.state == 1}">
                <div class="normal-div">
                我的会员级别: <span class="number">${membership.memberAccount.vipLevel.name}</span>
                我的优惠：<span class="number">${membership.memberAccount.vipLevel.discount*100}</span> 折
                </div>
            </c:if>

            <div class="normal-div">我的账户余额：
                <span class="number">${membership.memberAccount.balance}>
                    <button class="button" onclick="window.location.href='/user/recharge'">立即充值</button>
                </span>
            </div>

            <div class="normal-div">我的积分剩余：
                <span class="number">${membership.memberAccount.credit}
                    <button class="button" onclick="window.location.href='/user/credit'">积分兑换</button>
                </span>
            </div>
                    ...
                    <c:when test="${membership.state == 1}">
                        <div class="normal-div">您目前处于<span class="number">有效</span>状态。
                            如果到<span class="number">
                                <fmt:formatDate value="${membership.memberState.pauseTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
                            </span>时您的账户余额不足100元，您的账户将会变为<span class="number">暂停</span>状态。
                        </div>
                    </c:when>
                    <c:when test="${membership.state == 2}">
                        <div class="normal-div">您目前处于<span class="number">暂停</span>状态。
                            如果您不充值余额，您的账户将于<span class="number">
                                <fmt:formatDate value="${membership.memberState.stopTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
                            </span>停止使用。
                        </div>
                    </c:when>
                </c:choose>
